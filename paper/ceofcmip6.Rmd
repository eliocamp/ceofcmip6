...
The models and number of members per experiment used in this study are listed in Table \@ref(tab:mdoelos).
We used all available CMIP6 models with 5 or more members in the "historical" experiment and all available DAMIP models in the experiments containing only the effect of greenhouse gases ("hist-GHG"), natural variability nic forcings ("hist-nat"), anthropogenic aerosol forcings ("hist-aer") or only the effect of stratospheric ozone ("hist-stratO3") were used.

The historical experiment includes all known external forcings and is designed to evaluate the ability of the models to correctly simulate present climate and observed climate change.
In contrast, the rest of the experiments considered leave a certain forcing variable while fixing the remaining forcings.
The hist-nat experiments are similar but only include solar and volcanic forcing, representing the climate response in the absence of anthropogenic forcing.
...
The hist-aer simulations include only the forcing of changes in anthropogenic aerosol concentrations.
On the other hand, hist-stratO3 simulations include only the forcing of stratospheric ozone variation, leaving all other atmospheric chemistry and tropospheric ozone concentrations constant.
...
That is, this method considers $k$ simulations of $n$ years as a single simulation of $k\times n$ years.
Then, the cEOFs were calculated following the methodology of Section \@ref(ceof-method).
Then for each model and experiment a single (complex) spatial pattern per cEOF but in addition a (complex) time series per member was obtained.

To make it comparable to ERA5, cEOFs were computed for the modern period, between 1979 and 2014 (the latest year available for all members).

As explained above, cEOFs are not unequivocally defined as they accept any rotation in the complex plane analogously to how EOFs accept sign changes.
The cEOFs computed in ERA5 were rotated to maximize the correlation with either stratospheric ozone or ENSO as described in Section \@ref(ceof-method).
For the CMIP models, the cEOFs were rotated to maximize the spatial correlation of the patterns with the corresponding ERA5 cEOF. This looks for the pattern location to be similar to the observed one.
...
Prior to other analyses, it was decided to evaluate the ability of the models to capture the properties of the observed cEOFs.
For this, historical experiments were considered.

Figure \@ref(fig:compare-r2) shows the $r^2$ of the spatial patterns of each model for the two cEOFs.
In general these values range between `r combine_words(scales::percent(range(cors[experiment == "historical"]$r2)))`.
In all cases, the correlation between simulated and observed cEOF1 is higher than that obtained for cEOF2.
Although models with lower correlation with the observed modes can be identified, such as `r cors[experiment == "historical"][, model := reorder(model, r2, fun = mean)][, levels(model)[1:2]] %>% combine_words()`, even these are associated with values greater than 50%.

(ref:compare-r2-cap) Coefficient of determination ($r^2$) of the spatial patterns of each model with ERA5 for cEOF1 (red) and cEOF2 (blue).
...
To better understand the spatial patterns represented by each of the models, Figs.
\@ref(fig:all-ceof1) and \@ref(fig:all-ceof2) show the 0° phase of cEOF1 and cEOF2, respectively, with the models ordered according to the $r^2$ of the respective cEOF and those represented by ERA5.
For cEOF1 (Fig. \@ref(fig:all-ceof1)) all models except FGOALS-g3 (panel t) correctly capture the observed wave 1 pattern.
The differences with ERA5 are visually minimal, as expected from the high spatial correlation of these patterns.
For cEOF2 (Fig. \@ref(fig:all-ceof2)), all models capture the wave 3 pattern located in the Pacific-Atlantic sector.
In particular, the positive center south of South America and the negative centers to the sides of it coincide in all models with the centers of ERA5.
The main feature of the models with low correlation is the lower intensity and poor localization of the maximum located south of New Zealand.

The multi-model mean pattern of the phase of each cEOF at each level, calculated by averaging the spatial pattern corresponding to each of the models is shown in Figure \@ref(fig:mmm), along with the $r^2$ of these patterns with respect to those observed.
The patterns are extremely similar to those observed in ERA5 and have $r^2$ of 94% and 89% for cEOF1 and cEOF2 respectively.
The multi-model average is more similar to the observed patterns than any individual model, indicating that the deficiencies of each model are compensated for by averaging.
...
(ref:mmm-cap) Multimodel mean (shaded, positive values in red, negative in blue) of the spatial fields of each cEOF, phase and level. Contours mark ERA5 patterns (positive values in filled lines, negative values in dashed line). The $r^2$ between ERA5 and the multimodel mean is in parentheses. Arbitrary units.

The CMIP6 models then successfully capture the spatial patterns of the cEOFs in both the multi-model mean and the individual models.
Next is to explore whether the models succeed in capturing second-order features, such as their variability and relationship to other parts of the climate system.

## Tropical variability

As shown in Section \@ref(sources-ceof), cEOF2 is strongly related to tropical variability.
A first approach to explore this relationship in the CMIP6 models was to calculate the correlation between cEOF2 and ONI for each model (Figure \@ref(fig:cor-enso-plot)).
It was found that almost all models have a high correlation between ONI and the 90° phase of cEOF2 and almost no correlation between ONI and the 0° phase of cEOF2, which is consistent with observations.
However, there is great variability in the ability of models to capture this relationship.
Some, such as MIROC6 and CESM2 have a correlation close to that observed, but in most it is much lower.
IPSL-CM6A-LR and INM-CM5-0 show virtually no relationship between cEOF2 and ONI.
...
(ref:cor-enso-plot-cap) Coefficient of determination ($r^2$) between the ONI index and the 0° (blue) and 90° (red) phases of cEOF2 for each CMIP6 model and for ERA5.
...
The relationship between cEOF2 and ENSO is explained by a phase preference when the tropical forcing is active.
To evaluate this in CMIP6 models, Figure \@ref(fig:enso-phase-cmip) shows the relationship between ENSO and cEOF2 phase for CMIP6 models and Figure \@ref(fig:arg-enso-density) the distribution of cEOF2 phases for springs with ONI less than -0.5, greater than 0.5 and intermediate values.
Models with a high correlation between cEOF2 and ENSO show a strong sinusoidal relationship, while in models with low correlation the relationship is flatter.
The phase distributions according to ONI index categories (Fig. \@ref(fig:arg-enso-density)) are well separated in observations and in models with high correlation between cEOF2 and ONI.
In these models (e.g. MIROC6, CESM2), when ENSO is active (defined as ONI \> 0.5 or ONI \< -0.5), the phase of cEOF2 is concentrated near \$\pm\$0.5° (depending on the sign of ONI) whereas when ENSO is not as active (defined as ONI between -0.5 and 0.5), the phase distribution is more uniform.
...
(ref:fft-ceof2-cap) Fourier spectra in each model for the 90° phase of cEOF2 (orange), for the 0° phase of cEOF2 (violet) and for ONI (black). Dark line is the average spectrum of all members, shown in translucent lines. The ONI spectrum is the average spectrum of all members of each model. Panels are ordered from largest to smallest according to the $r^2$ between the 90° phase of cEOF2 and ONI, which is shown in parentheses in the title of each panel.

The relationship between cEOF2 and ENSO is also evident in the similarity of the periodogram of the series.
Figure \@ref(fig:fft-ceof2) shows the periodogram for cEOF2 with one line per limb and a thick line marking the average periodogram, as well as the average ONI periodogram of each model.
Most models have an ONI periodicity of 3 years similar to that observed in ERA5, although the intensity and maximum period vary significantly.

All of the models that have a clear periodicity at \~3 yr in the 90° phase of cEOF2 also have a very clear ENSO periodicity and also tend to have a correlation between the 90° phase of cEOF2 and ENSO higher.
On the other hand, none of the models with very low correlation with ENSO but clear ENSO periodicity have clear periodicity in cEOF2.
...
MRI-ENSM2-0, UKESM1-0-LL, MPI-ESM1-2-LR are some examples.

These observations suggest that ENSO is the source of cEOF2 periodicity in CMIP6 models but that their ability to represent observed periodicity depends not only on ENSO periodicity and the degree of correlation between the indices.
...
To study this relationship in more detail, the relationship between cEOFs and SST anomalies was evaluated.
Figure \@ref(fig:sst-mmm) shows the multi-model mean of the regression between SST and the two phases of each cEOF, marking the areas where more than half of the models have p-values less than 0.01.
The CMIP6 models reproduce the regression patterns of the 90° phase of cEOF2 relatively well.
...
(ref:cor-sst-regr-cap) Coefficient of determination ($r^2$) between the regression patterns of the 0° (purple) and 90° (orange) phases of each cEOF and SST each model and the respective regression pattern of ERA5.
...
To have a global and quantitative view of this comparison, Figure \@ref(fig:cor-sst-regr) shows the $r^2$ between the SST regressions associated with the 90° phase of cEOF2 for each model and the corresponding regression for ERA5.
For most models the pattern is similar to that observed, except for INM-CM5-0.
The regressions associated with the 0º phase of cEOF2, on the other hand, have low similarity to that observed in almost all models.
Possibly this is due to the excess signal present in the ENSO region.
...
(ref:series-largas-cap) Time series of standardized anomalies of the cEOFs computed using the period 1850 - 2014 averaged for each model (translucent lines) and their multi-model mean (dark line). Anomalies are computed with respect to the mean between 1850 and 1900. Translucent lines show the series for each model.

Figure \@ref(fig:extended-series) shows the time series over the entire period.
The 0° phase of cEOF1 has a small positive trend starting around `r trend_base`, consistent with the trend observed in ERA5 (Fig. \@ref(fig:extended-series)).
However, the 90° phase of cEOF1 has a much larger negative trend, which is not present in ERA5.
...
(ref:trends-ceof1-cap) Linear trends of each phase of cEOF1 from `r trend_base`. Each point represents a member, where members with significant trends (p-value \< 0.01) are marked with a cross. The dashed vertical line represents the average trend of all models.

The trends of each phase of cEOF1 for each model from `r trend_base` are shown in Figure \@ref(fig:trends-ceof1) along with the average trend of all models.
The average trend for the 0° phase is positive but very small.
In addition, the models are not consistent in their trends and only a few models have a positive average trend.
On the other hand, the trends of the 90º phase are more consistent.

These trends are incompatible with the observed cEOF1 variability, which has a positive trend in its 0º phase and no trend in its 90º phase.
It is possible that the models have some fundamental problem in capturing long-term variability in this way, either because of flaws in the internal dynamics or because of some problem with the forcing involved.

It is also possible that the observed trend is due to low-frequency internal variability.
...
(ref:mapa-trend-cap) Geopotential height trends (m per decade) at 50 hPa concordant with the trend of the 0º and 90º phase of cEOF1 and their sum.

The 90° phase of cEOF1, shows a positive trend in hist-aer and negative in hist-GHG and, also negative but weaker, in hist-stratO3.
Decreasing stratospheric ozone and increasing GHG concentrations contribute to the negative trend.
Both effects tend to reduce the geopotential height by 50 hPa over East Antarctica (Fig. \@ref(fig:map-trend) column 2).
...
For the 0° phase of cEOF1, neither hist-nat nor hist-aer show significant trends, suggesting that the observed trend is not due to variability or anthropogenic aerosol forcing.
...
The effect of the 0° phase trend of cEOF1 in the hist-GHG experiment on the 50 hPa geopotential height is a slight negative anomaly over the Ross Sea (Fig. \@ref(fig:map-trend) b.1).
This pattern would indicate an increasing jet to the south of New Zealand.
...
As a first approximation to the combined effect of all these forcings, Figure \@ref(fig:sum) shows the multi-model mean of the historical run together with the sum of the multi-model means of the hist-GHG, hist-stratO3 and hist-aer runs.
...
The CMIP6 models manage to characterize the spatial structure of the cEOFs satisfactorily, with good correlation between spatial patterns, particularly for the multi-model mean.
